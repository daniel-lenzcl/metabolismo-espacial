...
{
    public IReadOnlyDictionary<int, cPessoa> Individuos => registroAgentes;
    ...
    public void CriarPessoas(int total_pessoas)
    {
        ...
        foreach (var kv in quantidades)
        {
            string tipoId = kv.Key;
            int quantidade = kv.Value;

//            int quantidade = Mathf.RoundToInt(total_pessoas * percentual);
            DebugController.Log(DebugCategoria.GestorPopulacao, $"popular ▸ % {kv.Value}  gerando {quantidade} pessoas do tipo: {tipoId}");
    ...
    public Dictionary<string, int> CalcularQuantidadeHibrida(int totalAgentes)
    {
        ...
        // 1. Parte inteira
        foreach (var kv in percentualPorTemplate)
        {
            string tipoId = kv.Key;
            float percentual = kv.Value / 100f;
    ...
    private string SortearTipoPonderado()
    {
        ...
        return percentualPorTemplate.Keys.Last();
    }
